...
write.csv(bumedata, file = "bumedata_behav_mrs_20200530.csv")
...
table(bumedata.baseline$group, bumedata.baseline$severity_CARS)
prop.test(table(bumedata.baseline$group, bumedata.baseline$severity_CARS), correct = FALSE)
...
##### ADOS
### baseline comparison
print(colnames(bumedata)[17:21])
desmat <- matrix(rep(0,16*5), ncol = 16, nrow = 5)
rownames(desmat) <- colnames(bumedata.baseline)[c(17:21)]
colnames(desmat) <- c("n","min", "max", "mean", "sd",
                      "n","min", "max", "mean", "sd",
                      "t.df", "t.t", "t.p",
                      "kw.df", "kw.chi-squared", "kw.p")
...
print(desmat)
write.xlsx(desmat, file="output_rct_behav119.xls", sheetName = "baselinebehaviourADOS", append = T)

##### ADOS
###  follow-up
desmat <- matrix(rep(0,16*5), ncol = 16, nrow = 5)
rownames(desmat) <- colnames(bumedata.3mont)[c(17:21)]
colnames(desmat) <- c("n","min", "max", "mean", "sd",
                      "n","min", "max", "mean", "sd",
                      "t.df", "t.t", "t.p",
                      "kw.df", "kw.chi-squared", "kw.p")
...
print(desmat)
write.xlsx(desmat, file="output_rct_behav119.xls", sheetName = "month3behaviourADOS", append = T)
...
# # for those with the shaptio or the levent test being significant, we need to do permutation 
# # LMM: liner mixed effect model with nuisance variables
stats3 <- data.frame(t = rep(0,5), t.p = rep(1,5), f=rep(0,5), f.p=rep(1,5), f.df=rep(1,5), eta.sq.partial=rep(1,5))
rownames(stats3) <- colnames(bumedata[17:21])

print(colnames(bumedata)[17:21])
for (i in 17:21){
  score <- bumedata[,i]
  fit <- lmer( score ~  stage + group + group*stage  + (1|id), data=bumedata)
  stats3$shapitestp[i-16] <- shapiro.test(residuals(fit))$p.value
  stats3$leventestp[i-16] <- leveneTest(score ~ as.factor(group*stage), data=bumedata)[1,3]
  stats3$t[i-16] <- summary(fit)$coefficients["stage:group","t value"]
  stats3$t.p[i-16] <- summary(fit)$coefficients["stage:group","Pr(>|t|)"]
  stats3$t.df[i-16] <- summary(fit)$coefficients["stage:group","df"]
  ftests <- anova(fit, type=1, ddf="Kenward-Roger")
  stats3$f[i-16] <- ftests["stage:group","F value"]
  stats3$f.p[i-16] <- ftests["stage:group","Pr(>F)"]
  stats3$f.df[i-16] <- ftests["stage:group","DenDF"]
  stats3$eta.sq.partial[i-16] <- eta_sq(fit, partial = TRUE)[3,2] 
}

print(stats3)
write.xlsx(stats3, file="output_rct_behav119.xls", sheetName = "drugeffectonbehaviourModel3ADOS", append = T)



# # permutation test for subscales
currentdata <- bumedata
perm.p.ados <- matrix(rep(0,5), ncol=1, nrow=5)
rownames(perm.p.ados) <- colnames(currentdata)[c(17:21)]

for (i in rownames(perm.p.ados)){
  score <- (currentdata[[i]])
  # linear model comparison
  fit1 <- lmer(score ~ stage * group + stage + group + (1|id), data = currentdata)
  fit0 <- lmer(score ~ stage + group + (1|id), data = currentdata)
  perm.p.ados[[i,1]] <- permlmer(fit0, fit1, perms = 3000, ncore = 8, plot = FALSE)$"Perm-p"[2]
}

print(perm.p.ados)

# suggested by our open-label RCT findings, we focused on the following 6 
perm.p.ados.fdr  <- p.adjust(perm.p.ados, method = "fdr") 
print(perm.p.ados.fdr)

write.xlsx(cbind(perm.p.ados, perm.p.ados.fdr), file="output_rct_behav119.xls",
           sheetName = "drugbehModel3PermFDRADOS", append = T)


##### RBS-R
### baseline comparison
print(colnames(bumedata)[63:69])
desmat <- matrix(rep(0,16*7), ncol = 16, nrow = 7)
rownames(desmat) <- colnames(bumedata.baseline)[c(63:69)]
colnames(desmat) <- c("n","min", "max", "mean", "sd",
                      "n","min", "max", "mean", "sd",
                      "t.df", "t.t", "t.p",
                      "kw.df", "kw.chi-squared", "kw.p")
for (i in 63:69){
  # descriptive table:: desmat
  description <- describeBy(bumedata.baseline[,i], group = bumedata.baseline$group, mat = TRUE, na.rm = TRUE )
  desmat[i-62,c(1:10)] <- as.matrix(cbind(description[1, c("n","min", "max", "mean", "sd")],
                                          description[2, c("n","min", "max", "mean", "sd")]))
  # baseline comparison: testmat
  ttest <- t.test(bumedata.baseline[,i]~group, data = bumedata.baseline)
  desmat[i-62,11] <- ttest$parameter # degree-of-freedom
  desmat[i-62,12] <- ttest$statistic # t-statistic
  desmat[i-62,13] <- ttest$p.value #p value
  kwtest <- kruskal.test(bumedata.baseline[,i]~group, data = bumedata.baseline)
  desmat[i-62,14] <- kwtest$parameter # degree-of-freedom
  desmat[i-62,15] <- kwtest$statistic # t-statistic
  desmat[i-62,16] <- kwtest$p.value #p value
}

print(desmat)
write.xlsx(desmat, file="output_rct_behav119.xls", sheetName = "baselinebehaviourRBS", append = T)


##### RBS-R
### follow-up
desmat <- matrix(rep(0,16*7), ncol = 16, nrow = 7)
rownames(desmat) <- colnames(bumedata.3mont)[c(63:69)]
colnames(desmat) <- c("n","min", "max", "mean", "sd",
                      "n","min", "max", "mean", "sd",
                      "t.df", "t.t", "t.p",
                      "kw.df", "kw.chi-squared", "kw.p")
...
print(desmat)
write.xlsx(desmat, file="output_rct_behav119.xls", sheetName = "month3behaviourRBS", append = T)
...
# # for those with the shaptio or the levent test being significant, we need to do permutation 
# # LMM: liner mixed effect model with nuisance variables
stats3 <- data.frame(t = rep(0,7), t.p = rep(1,7), f=rep(0,7), f.p=rep(1,7), f.df=rep(1,7), eta.sq.partial=rep(1,7))
rownames(stats3) <- colnames(bumedata[63:69])

print(colnames(bumedata)[63:69])
for (i in 63:69){
  score <- bumedata[,i]
  fit <- lmer( score ~  stage + group + group*stage  + (1|id), data=bumedata)
  stats3$shapitestp[i-62] <- shapiro.test(residuals(fit))$p.value
  stats3$leventestp[i-62] <- leveneTest(score ~ as.factor(group*stage), data=bumedata)[1,3]
  stats3$t[i-62] <- summary(fit)$coefficients["stage:group","t value"]
  stats3$t.p[i-62] <- summary(fit)$coefficients["stage:group","Pr(>|t|)"]
  stats3$t.df[i-62] <- summary(fit)$coefficients["stage:group","df"]
  ftests <- anova(fit, type=1, ddf="Kenward-Roger")
  stats3$f[i-62] <- ftests["stage:group","F value"]
  stats3$f.p[i-62] <- ftests["stage:group","Pr(>F)"]
  stats3$f.df[i-62] <- ftests["stage:group","DenDF"]
  stats3$eta.sq.partial[i-62] <- eta_sq(fit, partial = TRUE)[3,2] 
}

print(stats3)
write.xlsx(stats3, file="output_rct_behav119.xls", sheetName = "drugeffectonbehaviourModel3RBS", append = T)



# # permutation test for subscales
currentdata <- bumedata
perm.p.rbs <- matrix(rep(0,7), ncol=1, nrow=7)
rownames(perm.p.rbs) <- colnames(currentdata)[c(63:69)]

for (i in rownames(perm.p.rbs)){
  score <- (currentdata[[i]])
  # linear model comparison
  fit1 <- lmer(score ~ stage * group + stage + group + (1|id), data = currentdata)
  fit0 <- lmer(score ~ stage + group + (1|id), data = currentdata)
  perm.p.rbs[[i,1]] <- permlmer(fit0, fit1, perms = 3000, ncore = 8, plot = FALSE)$"Perm-p"[2]
}

print(perm.p.rbs)

# suggested by our open-label RCT findings, we focused on the following 6 
perm.p.rbs.fdr  <- p.adjust(perm.p.rbs, method = "fdr") 
print(perm.p.rbs.fdr)

write.xlsx(cbind(perm.p.rbs, perm.p.rbs.fdr), file="output_rct_behav119.xls",
           sheetName = "drugbehModel3PermFDRRBS", append = T)
...
######################################################################################### 
## AJP modefy: add-in analysis about NAA################################################# 
...
colnames(ICGABA) <- c("stage","group","id","ic_gaba_corr")
...
for (k in stage){
  for (g in group){
    for (j in MRS.trans){
      ...
      covariates <- cbind(dat$age, dat$sex, dat$cutoffDQ75IQ70)
      for (b in beh){
        s = s + 1
...
delta.tot <- bumedata$CARS_tot[bumedata$stage==1] - bumedata$CARS_tot[bumedata$stage==0]
group <- bumedata$group[bumedata$stage==0]
cohen.d(delta.tot, group)
describeBy(delta.tot, group)

data.paired <- bumedata[which(bumedata$QC.paired==TRUE),] 
group <- data.paired$group[data.paired$stage==0]
delta.ga <- data.paired$ic_gaba_corr[data.paired$stage==1]-data.paired$ic_gaba_corr[data.paired$stage==0]
cohen.d(delta.ga, group)
describeBy(delta.ga, group)
